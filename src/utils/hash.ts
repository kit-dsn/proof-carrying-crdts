...
/**
 * Calling update multiple times on initialState should always return the same result like calling {@code hash} with the given inputs as an array.
 * Invariant: {@code getHash((await prove_hash(input)).publicOutput) == hash(input)}
 *
 * It can also be used in other zkPrograms similar to how it is used in {@link HashRecursionProgram}.
 * @see {@file ../scripts/hash preimage/arbitrary.ts}
 */
