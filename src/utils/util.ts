import {Encoding, Field, Proof, setNumberOfWorkers} from 'o1js'
...
export async function writeProof(filePath: PathLike, proof: Proof<any, any>) {
    await fs.writeFile(filePath, JSON.stringify(proof.toJSON()))
}

export async function readProof(filePath: PathLike) {
...
export function start(filePath: string) {
    ...
    if (envVar !== undefined) {
        ...
        if (!isNaN(parsedValue)) {
            ...
            console.log(`Using ${parsedValue} workers.`)
        } else {
