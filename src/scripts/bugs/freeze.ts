...
import {readProof, writeProof} from "utils/util.js";
...
//let proof: Proof<void, Field> = await LinearRecursionProof.fromJSON(await readProof('data/crash/60.json'))
...
for (let i = 0; i < 500; i++) {
    ...
    await writeProof(`data/freeze/${i}.json`, proof)
}
