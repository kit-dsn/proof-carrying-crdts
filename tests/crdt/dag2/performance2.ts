...
if (action === 'compile') {
    ...
    if (action === 'base') {
        ...
        const base = (await Dag.base(update0.room_id, content, Field(1), Field(1))).proof
        await measureTiming(iterations, `${path}/next.txt`, () => Dag.next(base, proof0))
    } else if (action === 'verify') {
        const [update0, proof0] = await proveUpdate('first update', Field(0), [])
        await measureTiming(iterations, `${path}/verify.txt`, () => Dag.verify(proof0))
    }
}
